...
if(isset($_COOKIE['players'])){
	...
	$playas = explode(':', $_COOKIE['players']);
...
if(isset($_GET['player']) && $_GET['player'] != ''){
	...
	setcookie('players', implode(':', $players), time() + (3600 * 24 * 900));
}
elseif(isset($_GET['player_select'])){

	$player = $_GET['player_select'];
}
else{
...
foreach($players as $name){
	...
	echo '<option ' . ($name == $player ? 'selected' : '') . '>' . $name . '</option>';
}

echo '
	</select>
	<label><input type="checkbox" name="show_inactive" ' . (isset($_GET['show_inactive']) ? 'checked' : '') . '/>Show inactive assignments</label>
	<label><input type="checkbox" name="show_completed_assignments" ' . (isset($_GET['show_completed_assignments']) ? 'checked' : '') . '/>Show completed assignments</label>
	<label><input type="checkbox" name="show_completed_criteria" ' . (isset($_GET['show_completed_criteria']) ? 'checked' : '') . '/>Show completed criteria</label>
...
<div style="float: left;">
	<a href="http://bf3stats.com/stats_pc/' . $player . '" target="_blank">
		refresh
	</a>
	(wait untill top right says player has been updated, then refresh this page, can only be done once a day)
</div>
...
if($statuscode == 200){
	...
	echo '<pre><div class="js-masonry" data-masonry-options=\'{ "columnWidth": 1 }\'>';

	$unordered = array();

	foreach($assignments as $array1){

		foreach($array1 as $array2){

			$inactive = false;

			if($array2['count'] == 1 && !isset($_GET['show_completed_assignments'])){
			...
			$output .= '
				<div style="margin: 10px; border: solid 1px black; ' . ($inactive ? 'background-color: red;' : '') . '">';
			$output .= '<div style="height: 15px;"><b>' . $array2['name'] . '</b></div>';

			$count = 0;
			$totalperc = 0;
			$crits = '';

			foreach($array2['criteria'] as $criteria){

				$count++;

				$perc = ($criteria['curr'] / $criteria['needed']) * 100;
				$totalperc += $perc;

				if($criteria['curr'] == $criteria['needed'] && !isset($_GET['show_completed_criteria'])){

					continue;
				}

				$needed = $criteria['needed'] - $criteria['curr'];
				$perc = ($criteria['curr'] / $criteria['needed']) * 100;

				$name = str_replace($criteria['needed'], $needed, $criteria['nname']);

				$crits .= '<div style="height: 15px;">';
				$crits .= '<div style="height: 15px;">' . $name . '</div>';
				$crits .= '<div style="height: 15px; margin-top: -15px; background-color: lightgreen;  width: ' . $perc . '%"></div>';
				$crits .= '</div>';
			}

			$perc = $totalperc / $count;

			$output .= '<div style="height: 15px; margin-top: -15px; background-color: lightgreen;  width: ' . $perc . '%"></div>';
			$output .= $crits;
			$output .= '</div>';

			$unordered[] = array('perc' => $perc, 'output' => $output);
		}
	}

	usort($unordered, "sortUnordered");

	foreach($unordered as $ordered){
